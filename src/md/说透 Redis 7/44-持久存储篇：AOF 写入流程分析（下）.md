...
我们前面提到，在 serverCron() 中会定时执行 flushAppendOnlyFile() 函数，相关的调用位置如下图所示，很明显，第一个调用点判断了 aof\_flush\_postponed\_start 字段不为 0，是为了进行延迟写入；第二个调用点判断了 aof\_last\_write\_status 字段的值，是为了写入失败的重试。
