...
3.  因为表`t`的列`col`采用的是`gbk`字符集，与`character_set_connection`一致，所以直接到列中找字节值为`0xCED2`的记录，最后找到了一条记录。
    ...
    > 小贴士：  
    >   
    > 如果某个列使用的字符集和character\_set\_connection代表的字符集不一致的话，还需要进行一次字符集转换。
...
![image_1d6dfk4orjaj1ra8536aijb99.png-112.4kB](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f87857b928f84edca6a6cf127b3fda5a~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.image#?w=974&h=469&s=115067&e=png&b=ffffff)
...
> 小贴士：  
>   
> 列`col`中各个字符在使用gbk字符集编码后对应的数字如下： 'A' -> 65 （十进制） 'B' -> 66 （十进制） 'a' -> 97 （十进制） 'b' -> 98 （十进制） '我' -> 25105 （十进制）
